    ...
    async.waterfall([
        function (callback) {
            ...
            return callback(null, {hash: cachedData.hash, identifier: 'global', lastHash: Math.floor(cachedData.lastHash / 1000),
                totalHashes: cachedData.totalHashes, validShares: Number(cachedData.goodShares) || 0, invalidShares: Number(cachedData.badShares) || 0});
        },
...
app.get('/config', function (req, res) {
    res.json({
        ...
        payout_frequency: global.config.payout.timer,
        fee_slew_amt: global.config.payout.feeSlewAmount,
        fee_slew_end: global.config.payout.feeSlewEnd,
        pplns_multiplier: global.config.pplns.shareMulti,
        payout_frequency: global.config.payout.timer,
        fee_slew_amt: global.config.payout.feeSlewAmount,
        fee_slew_end: global.config.payout.feeSlewEnd,
        pplns_multiplier: global.config.pplns.shareMulti,
        coin_code: global.config.general.coinCode,
        block_target_time: global.config.coin.blockTargetTime
    });
});

// Pool APIs
app.get('/pool/topMiners', function (req, res) {
    let intCache = {};
    global.mysql.query("SELECT balance.pool_type, balance.last_mine, balance.payment_address as address, balance.payment_id as payment_id " +
        "FROM balance " +
        "WHERE pool_type <> 'fees' AND last_mine IS NOT NULL group by address, payment_id, pool_type, last_mine").then(function (rows) {
        ...
        if (minerList) {
            minerList.forEach(function (miner) {
                ...
                if (minerCache.hash !== 0) {
                    if (!intCache.hasOwnProperty(minerData[0])) {
                        intCache[minerData[0]] = {address: '*****'+minerData[0].slice(-5), workers: []};
                    }
            ...
            retList = retList.filter(function (miner) {
...
// Blocks per day + LAG x 30 days
app.get('/network/chart/difficulty', function (req, res) {
    let limit = typeof(req.query.limit) !== 'undefined' ? Number(req.query.limit) : 22050;
    let page = typeof(req.query.page) !== 'undefined' ? Number(req.query.page) : 0;
    let query = "SELECT * FROM block_log ORDER BY id DESC LIMIT ? OFFSET ?";
    let diffHisVal = [];
    global.mysql.query(query, [limit, page * limit]).then(function (rows) {
        rows.forEach(function (row) {
	        let ts = new Date(row.find_time);
            diffHisVal.push({
...
app.get('/miner/:address/stats/:identifier', function (req, res) {
    ...
    return res.json({
        ...
        hash: cachedData.hash,
        totalHash: cachedData.totalHashes,
        validShares: Number(cachedData.goodShares) || 0,
        invalidShares: Number(cachedData.badShares) || 0
    });
});
