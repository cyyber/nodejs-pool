    ...
    global.support.rpcWallet('getbalance', [], function (body) {
        if (body.result) {
            ...
            if (body.result.unlocked_balance < amountToPay) {
                console.error("Wallet only has " + body.result.unlocked_balance + " unlocked balance, can't pay " + amountToPay + " worth of LTHN. Retrying in " + timerRetry + " minutes!");
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                ...
                if (global.config.general.discordPayoutAnnounce === true) {
                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(amount)  + " LTHN to 1 miner");
                }
            } else {
                    ...
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > 10) {
                                ...
                                if (global.config.general.discordPayoutAnnounce === true) {
                                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(totalAmount)  + " LTHN to " + paymentDetails.destinations.length + " miners");
                                }
                            } else {
                                console.error("Unknown error from the wallet.");
                            }
