...
function templateUpdate(repeating) {
    ...
    async.waterfall([
        ...
        function (currentBlockHeight, callback) {
            let templateRefreshThreshold = Date.now() - (global.config.pool.templateRefreshTime * 1000);

            if (activeBlockTemplate && activeBlockTemplate.height === currentBlockHeight && activeBlockTemplate.timestamp > templateRefreshThreshold) {
                ...
                global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
                    if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
                        ...
                        if (!activeBlockTemplate || rpcResponse.height > activeBlockTemplate.height || (activeBlockTemplate && activeBlockTemplate.timestamp < templateRefreshThreshold)) {
...
function newBlockTemplate(template) {
    ...
    if (activeBlockTemplate && (activeBlockTemplate.height == template.height)) {
        console.log(threadName + 'Refreshing existing block template for height: ' + template.height);
    } else {
    ...
    activeBlockTemplate.timestamp = Date.now();
    for (let minerId in activeMiners) {
        if (activeMiners.hasOwnProperty(minerId)) {
            let miner = activeMiners[minerId];
