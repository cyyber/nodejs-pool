    ...
    for (let minerId in activeMiners) {
        if (activeMiners.hasOwnProperty(minerId)) {
            ...
            if (Date.now() - miner.lastContact > global.config.pool.minerTimeout * 1000) {
                ...
                activeMinerCount[miner.payout] -= 1;
            }
        }
    }
}
...
function templateUpdate(repeating) {
    // Calling getBlockTemplate repeatedly is incredibly taxing on Monero based blockchain daemons.
    // For *significant* CPU savings, we indirectly check if we're mining the correct block by calling
    // getLastBlockerHeader and inferring the height from there.
    async.waterfall([
        function (callback) {
            global.coinFuncs.getLastBlockHeader(function (err, blockHeader) {
                if (err !== null){
                    return callback(true, "Issue getting last block header: " + JSON.stringify(blockHeader));
                }
                callback(null, blockHeader.height + 1);
            });
        },
            ...
            if (activeBlockTemplate && activeBlockTemplate.height === currentBlockHeight) {
                ...
                global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
                    if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
                        ...
                        callback(null, rpcResponse.height)
                    } else {
                        if (repeating !== true) {
                            setTimeout(templateUpdate, 300);
                        }
                        callback(true, "Issue getting block template: " + JSON.stringify(rpcResponse));
                    }
                });
            }
        }
        ...
        if (err) {
            return console.error("Unable to get check for new block template: " + results);
        }
    });
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    if (global.coinFuncs.isIntegratedAddress(this.address) === true && this.paymentID) {
        this.error = "Integrated addresses cannot also have a Payment ID";
        this.valid_miner = false;
    }
    if (activeMinerCount.indexOf(this.payout) >= 0 && activeMinerCount[this.payout] >= global.config.pool.maxConnectionsPerMiner) {
        this.error = "Worker count exceeded ("+global.config.pool.maxConnectionsPerMiner+") please use a proxy for high worker counts";
        this.valid_miner = false;
    }
    if (!activeBlockTemplate) {
    ...
    this.checkBan = function (validShare) {
        ...
        if (this.validShares + this.invalidShares >= global.config.pool.banThreshold) {
            if (this.invalidShares / this.validShares >= global.config.pool.banPercent / 100) {
                let miner = activeMiners[this.id];
                delete activeMiners[this.id];
                process.send({type: 'removeMiner', data: miner.port});
                activeMinerCount[this.payout] -= 1;
        ...
        this.getJob = function () {
            ...
            if (!this.proxy) {
                ...
                this.cachedJob = {
                    blob: blob,
                    height: activeBlockTemplate.height,
                    algo: global.coinFuncs.variantShortName(activeBlockTemplate.height),
                    job_id: newJob.id,
                    target: target,
                    id: this.id
                };
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (global.config.pool.trustedMiners && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
        ...
        hash = global.coinFuncs.cryptoNight(convertedBlob, blockTemplate.height);
        shareType = false;
    }
    if (hash.toString('hex') !== resultHash) {
        let oldAlgo = global.coinFuncs.oldAlgorithmCheck(convertedBlob, resultHash, blockTemplate.height);
        if (oldAlgo !== null) {
            miner.algo_error = "Old algorithm in use (" + oldAlgo + "). Please update your miner to use: " + global.coinFuncs.variantName(convertedBlob[0]); 
            console.error(threadName + "Old algorithm from miner (" + oldAlgo + ") " + miner.logString);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)) {
        ...
        global.coinFuncs.submitBlock(shareBuffer.toString('hex'), function (err, status) {
            if (err !== null){
                // Did not manage to submit a block.  Log and continue on.
                console.error(threadName + "Error submitting block at height " + job.height + " from " + miner.logString + ", share type: " + shareType + " error: " + JSON.stringify(status.error));
                ...
                console.log(threadName + "Block " + blockFastHash.substr(0, 6) + " found at height " + job.height + " by " + miner.logString +
                    ", share type: " + shareType + " - submit result: " + status);
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            if (activeMinerCount.indexOf(miner.payout) >= 0) {
                activeMinerCount[miner.payout] += 1;
            }
            else {
                activeMinerCount[miner.payout] = 1;
        ...
        case 'submit':
            ...
            if (!shareAccepted) {
                if (miner.algo_error !== null) {
                    sendReply(miner.algo_error);
                    miner.algo_error = null;
                } else {
