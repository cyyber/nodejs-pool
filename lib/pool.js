...
let algo = global.coinFuncs.algoStr();
...
function processShare(miner, job, blockTemplate, params) {
    ...
    let shareBuffer = global.coinFuncs.constructNewBlob(template, new Buffer(nonce, 'hex'), blockTemplate);
    ...
    if (hashDiff.ge(blockTemplate.difficulty)) {
        ...
        global.coinFuncs.submitBlock(shareBuffer.toString('hex'), function (err, status) {
            if (err !== null){
                ...
                let blockFastHash = hash.toString('hex');
                ...
                if (global.config.general.discordBlockAnnounce === true) {
                    global.support.poolBotMsg("Hey hey, a miner with a wallet ending in: " + miner.address.slice(-5) + " has something to say...");
                }
            } else {
