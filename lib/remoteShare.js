        ...
        switch (msgData.msgType) {
            ...
            case global.protos.MESSAGETYPE.BLOCK:
                global.database.storeBlock(msgData.exInt, msgData.msg, function(data){
                    if (!data){
                        // Avoid storeBlock retry spam by delaying error response by 2 seconds
                        setTimeout(function () { return res.status(400).end() }, 2000);
                        return;
                    } else {
                        return res.json({'success': true});
                    }
...
if (cluster.isMaster) {
    ...
    app.listen(global.config.leafapi_port, function () {
