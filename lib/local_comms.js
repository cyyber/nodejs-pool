...
function Database(){
                ...
                for (let key in cachedData){
                    if (cachedData.hasOwnProperty(key)){
                        ...
                        if (cacheStore === null){
                            ...
                            if (cachedData[key].hasOwnProperty('roundHashes') && json_cache.hasOwnProperty('roundHashes') && !foundBlock){
    ...
    this.storeBlock = function(blockId, blockData, callback){
        ...
        try{
            ...
            global.coinFuncs.getBlockHeaderByHeight(blockId, function(err, header){
                if (err || typeof(header) === 'undefined' || !header){
                    // We only care if we're capable of verifying the hash. If it is an orphan, blockManager will sort it out for us
                    console.log("Master node is not yet synced to height " + blockId + ". Asking leaf node to re-submit block."); 
                    return callback(false);
                }
                blockDataDecoded.value = header.reward;
                blockData = global.database.calculateShares(blockDataDecoded, blockId);

                if (global.config.general.discordBlockAnnounce === true) {
                    // The shares get returned as part of the encoded blockData above, so to easily access them
                    // we'll decode blockData again now to use it for our poolBotMsg
                    var blockDataDecodedForBot = global.protos.Block.decode(blockData);
                    var blockEffort = Math.round(blockDataDecodedForBot.shares / blockDataDecoded.difficulty * 100);
                   if (blockEffort >= 1500) {
                        global.support.poolBotMsg("Oh fuck off !!!   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 1100) {
                        global.support.poolBotMsg(":middle_finger: :middle_finger: :middle_finger:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 900) {
                        global.support.poolBotMsg(":middle_finger: :middle_finger:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 700) {
                        global.support.poolBotMsg(":middle_finger:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 500) {
                        global.support.poolBotMsg(":poop:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 400) {
                        global.support.poolBotMsg(":angry:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 300) {
                        global.support.poolBotMsg(":sob:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort >= 200) {
                        global.support.poolBotMsg(":sweat:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort > 100) {
                        global.support.poolBotMsg(":neutral_face:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort <= 5) {
                        global.support.poolBotMsg(":money_mouth:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort <= 10) {
                        global.support.poolBotMsg(":joy:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort <= 25) {
                        global.support.poolBotMsg(":laughing:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort <= 50) {
                        global.support.poolBotMsg(":wink:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort < 100) {
                        global.support.poolBotMsg(":slight_smile:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    } else if (blockEffort = 100) {
                        global.support.poolBotMsg(":smile:   <@&639401668022239232> Block " + blockId + " mined | Reward: " + global.support.coinToDecimal(blockDataDecoded.value) + " " + global.config.general.coinCode + " | Effort: " + blockEffort + "%");
                    }
                }
                let txn = global.database.env.beginTxn();
                ...
                global.coinFuncs.getLastBlockHeader(function(err, body){
                    if (oldestLockedBlock === null){
                        ...
                        if (global.config.pps.enable){
                            ...
                            callback(null, body.height - Math.floor(global.config.payout.blocksRequired/2), Math.floor(body.difficulty * Math.max(global.config.pplns.shareMulti * 1.3, global.config.pplns.minEffortRetention / 100)));
                        } else {
                            // Otherwise, we can just start from the current height.  Woo!
                            callback(null, body.height, Math.floor(body.difficulty * Math.max(global.config.pplns.shareMulti * 1.3, global.config.pplns.minEffortRetention / 100)));
                        }
                        ...
                        if (global.config.pps.enable && oldestLockedBlock.height > body.height - Math.floor(global.config.payout.blocksRequired/2)) {
                            ...
                            callback(null, oldestLockedBlock.height, Math.floor(oldestLockedBlock.difficulty * Math.max(global.config.pplns.shareMulti * 1.3, global.config.pplns.minEffortRetention / 100)));
                        }
                    }
                });
            },
