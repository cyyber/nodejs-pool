...
let balanceQueue = async.queue(function (task, callback) {
        ...
        async.waterfall([
                ...
                function (balance_id, intCallback) {
                    ...
                    global.mysql.query("UPDATE balance SET amount = amount+?, last_mine = NOW() WHERE id = ?", [amount, balance_id]).then(function () {
...
function calculatePPLNSPayments(blockHeader) {
    ...
    if (global.config.general.discordBlockAnnounce === true) {
        global.support.poolBotMsg(":raised_hand:   PPLNS Calculating | Block: " + blockHeader.height + " | Value: " + global.support.coinToDecimal(blockHeader.reward));
    }
    let rewardTotal = blockHeader.reward;
    ...
    async.doWhilst(function (callback) {
        ...
        if (global.config.general.discordBlockAnnounce === true) {
            global.support.poolBotMsg(":thumbsup:   PPLNS Calculated | Block: " + blockHeader.height + " | Payout: " + global.support.coinToDecimal(totalPayments) + " (" + (totalPayments / blockHeader.reward) * 100 + "%)");
        }
    });
}
...
function calculateSoloPayments(blockHeader) {
    ...
    if (global.config.general.discordBlockAnnounce === true) {
        global.support.poolBotMsg(":raised_hand:   SOLO Calculating | Block: " + blockHeader.height + " | Value: " + global.support.coinToDecimal(blockHeader.reward));
    }
    let txn = global.database.env.beginTxn({readOnly: true});
    let cursor = new global.database.lmdb.Cursor(txn, global.database.shareDB);
    let paymentData = {};
    ...
    if (global.config.general.discordBlockAnnounce === true) {
        global.support.poolBotMsg(":thumbsup:   SOLO Calculated | Block: " + blockHeader.height + " | Payout: " + global.support.coinToDecimal(totalPayments) + " (" + (totalPayments / blockHeader.reward) * 100 + "%)");
    }
}
...
function blockUnlocker() {
    ...
    global.coinFuncs.getLastBlockHeader(function (err, blockHeader) {
        if (err !== null) {
            console.error(`Unable to run block unlocker due to: ${JSON.stringify(blockHeader)}`);
        }

        let blockHeight = blockHeader.height;

        blockList.forEach(function (row) {
            global.coinFuncs.getBlockHeaderByHeight(row.height, function (err, blockHeader) {
                if (blockHeader.hash !== row.hash) {
                    global.database.invalidateBlock(row.height);
                    global.mysql.query("UPDATE block_log SET orphan = 1 WHERE hex = ?", [row.hash]);
                    blockIDCache.splice(blockIDCache.indexOf(blockHeader.height));
                    console.log("Invalidating block " + blockHeader.height + " due to being an orphan block");
                    if (global.config.general.discordBlockAnnounce === true) {
                        global.support.poolBotMsg(":rage:   Block " + blockHeader.height + " was orphaned by the network");
                    }
                } else {
